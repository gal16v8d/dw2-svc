...
public class RestValidationHandler extends ResponseEntityExceptionHandler {
	...
	private static final String UNIQUE_INDEX_OR_PRIMARY_KEY_VIOLATION = "Unique index or primary key violation";
	...
	protected ResponseEntity<ApiError> handleConflict(ConstraintViolationException e) {
		...
		return ResponseEntity.badRequest().body(ApiError.builder().message(messages.toString()).build());
	}

	@ExceptionHandler(Exception.class)
	protected ResponseEntity<ApiError> handleConflict(Exception e) {
		boolean indexViolated = Arrays.toString(e.getStackTrace()).contains(UNIQUE_INDEX_OR_PRIMARY_KEY_VIOLATION);
		if (indexViolated) {
