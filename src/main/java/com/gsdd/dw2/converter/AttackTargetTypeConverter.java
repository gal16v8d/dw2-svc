...
public class AttackTargetTypeConverter
  ...
  public AttackTargetType convertToEntity(AttackTargetTypeModel model) {
    return Optional.ofNullable(model).map(m -> AttackTargetType.builder()
        .attackTargetTypeId(m.getAttackTargetTypeId()).name(m.getName()).build()).orElse(null);
  }
  ...
  public AttackTargetType mapToEntity(AttackTargetTypeModel model, AttackTargetType oldEntity) {
    ...
    newEntity.setName(Optional.ofNullable(model).map(AttackTargetTypeModel::getName)
