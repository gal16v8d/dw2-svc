...
class AttackConverterTest {
	...
	private Attack arrangeAttack() {
		return Attack.builder().attackId(1L).mp(0).name(NECRO_MAGIC)
				...
				.attackTargetType(AttackTargetType.builder().attackTargetTypeId(1L).name(ALL).build()).build();
	}
	...
	private AttackModel arrangeAttackModel() {
		return AttackModel.builder().attackId(1L).mp(0).name(NECRO_MAGIC).attackTypeId(1L).attackTargetTypeId(1L)
				.build();
	}
	...
	void convertToEntityTest() {
		willReturn(Optional.ofNullable(AttackType.builder().attackTypeId(1L).build())).given(typeRepo).findById(eq(1L));
		willReturn(Optional.ofNullable(AttackTargetType.builder().attackTargetTypeId(1L).build())).given(targetRepo)
				.findById(eq(1L));
