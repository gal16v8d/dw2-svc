...
class EvolutionServiceIT {
	...
	void saveTest() {
		EvolutionModel result = service.save(
				EvolutionModel.builder().dnaTimes(TO_ULTRA_MEGA).baseDigimonId(1400L).evolvedDigimonId(1600L).build());
		Assertions.assertNotNull(result);
	...
	void saveNameExistsTest() {
		Assertions.assertThrows(DataIntegrityViolationException.class, () -> service.save(
				EvolutionModel.builder().dnaTimes(ZERO_OR_MANY).baseDigimonId(1400L).evolvedDigimonId(1600L).build()));
	}

	@Test
	void updateTest() {
		EvolutionModel result = service.update(TO_MEGA_ID, EvolutionModel.builder().dnaTimes(TO_ULTRA_MEGA).build());
		Assertions.assertNotNull(result);
		Assertions.assertEquals(TO_ULTRA_MEGA, result.getDnaTimes());
	}
	...
	void updateNoMatchTest() {
		Assertions.assertNull(service.update(ID_NOT_FOUND,
				EvolutionModel.builder().dnaTimes(TO_ULTRA_MEGA).baseDigimonId(1400L).evolvedDigimonId(100L).build()));
	}

	@Test
	void patchTest() {
		EvolutionModel result = service.patch(TO_MEGA_ID,
				EvolutionModel.builder().dnaTimes(TO_ULTRA_MEGA).baseDigimonId(1400L).evolvedDigimonId(1600L).build());
		Assertions.assertNotNull(result);
		Assertions.assertEquals(TO_ULTRA_MEGA, result.getDnaTimes());
	}
