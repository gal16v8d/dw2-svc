...
class AttackTargetTypeControllerTest {
  ...
  void updateTest() throws Exception {
    AttackTargetTypeModel model =
        AttackTargetTypeModel.builder().attackTargetTypeId(1L).name(RANDOM_ALLY).build();
    willReturn(model).given(attackTargetTypeService).update(anyLong(),
        any(AttackTargetTypeModel.class));
  ...
  void updateNotFoundTest() throws Exception {
    AttackTargetTypeModel model =
        AttackTargetTypeModel.builder().attackTargetTypeId(1L).name(RANDOM_ALLY).build();
    willReturn(null).given(attackTargetTypeService).update(anyLong(),
        any(AttackTargetTypeModel.class));
    mvc.perform(MockMvcRequestBuilders.put(V1_ATTACK_TYPES_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
  ...
  void patchNotFoundTest() throws Exception {
    AttackTargetTypeModel model = AttackTargetTypeModel.builder().name(RANDOM_ALLY).build();
    willReturn(null).given(attackTargetTypeService).patch(anyLong(),
        any(AttackTargetTypeModel.class));
    mvc.perform(MockMvcRequestBuilders.patch(V1_ATTACK_TYPES_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isNotFound());
