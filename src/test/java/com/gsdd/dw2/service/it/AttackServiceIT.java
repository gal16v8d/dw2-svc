...
class AttackServiceIT {
  ...
  void saveNameExistsTest() {
    Assertions.assertThrows(DataIntegrityViolationException.class,
        () -> service.save(AttackModel.builder().name(ATTACK).mp(4).attackTypeId(ATTACK_ID)
            .attackTargetTypeId(ATTACK_TARGET_ID).build()));
  }

  @Test
  void updateTest() {
    AttackModel result = service.update(INTERRUPT_ID, AttackModel.builder().name(RUN).mp(0)
        .attackTypeId(ATTACK_ID).attackTargetTypeId(ATTACK_TARGET_ID).build());
    Assertions.assertNotNull(result);
    Assertions.assertEquals(RUN, result.getName());
  }
